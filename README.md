...
│
├── tests/                             # 34 pytest tests on synthetic data (no download needed)
│   ├── conftest.py
│   ├── test_loader.py
...
│
└── outputs/
    ├── figures/                       # 13 notebook figures used by the report (pipeline/ is gitignored)
...
Artefacts: `models/saved/`, `data/processed/`, `outputs/results/pipeline_*.csv`, `outputs/figures/pipeline/`.
...
The notebook is **self-contained**: it doesn't import `src/`, so it runs as-is in Google Colab or from inside a submission zip. It follows the same order as the report: *Data Loading → Data Understanding & EDA → Data Preparation → Modeling → Evaluation → Conclusion*.
...
```bash
pytest                                  # 34 tests, ~5 s, synthetic fixtures (no dataset download)
